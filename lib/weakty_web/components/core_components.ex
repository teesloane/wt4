defmodule WeaktyWeb.CoreComponents do
  @moduledoc """
  ...
  def page_container(assigns) do
    ...
    <div class={[@class, "max-w-#{@size}"]}>
      <h1 :if={@title} class="text-xl font-normal my-8 text-center uppercase prose tracking-wide averia">
        <%= @title %>
      </h1>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(WeaktyWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(WeaktyWeb.Gettext, "errors", msg, opts)
    end
  end
